package Collection_Framework.Collection_Interfaces.SplitIterator;
...
/*                                             The Spliterator interface,
    ...
    introduced in Java 8, can be used for traversing and partitioning sequences. It's a base utility for Streams,
     ...
     Spliterator itself does not provide the parallel programming behavior.However, it provides some methods to support it.
         Developers should utilize Spliterator interface methods and implement parallel programming by using

             Fork/Join Framework (one good approach).
            ...
            * It uses
                  ...
                  method to iterate elements individually in multiple Threads to support Parallel Processing.

            * It uses

                   forEachRemaining()

                  method to iterate elements sequentially in a single Thread.

             * It uses

                   trySplit()

                  method to divide itself into Sub-Spliterators to support Parallel Processing.
...
/**                                                           SplitIterator
    ...
    A Spliterator may traverse elements individually ({@link  #tryAdvance tryAdvance()}) or sequentially in bulk ({@link
        #forEachRemaining forEachRemaining()}).

    A Spliterator may also partition off some of its elements (using  {@link #trySplit}) as another Spliterator, to be
        used in  possibly-parallel operations.Operations using a Spliterator that  cannot split, or does so in a highly
        imbalanced or inefficient manner, are unlikely to benefit from parallelism.Traversal and splitting exhaust elements;
        each Spliterator is useful for only a single bulk computation.
 *
 ...
 * {@link Spliterator.OfInt int}, {@link Spliterator.OfLong long}, and {@link Spliterator.OfDouble double} values.
...
interface Spliterator_<T> {
     ...
     *
     * @apiNote Most Spliterators for Collections_Class, that cover all elements of a
...
//    }
     ...
     * @since 1.8
     */
    public interface OfLong extends OfPrimitive<Long, LongConsumer, Spliterator.OfLong> {
...
//        }
     ...
     * @since 1.8
     */
    public interface OfDouble extends OfPrimitive<Double, DoubleConsumer, Spliterator.OfDouble> {
