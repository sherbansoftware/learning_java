package Collection_Framework.Concurrent_Collection_Classes.LinkedTransferQueue;
...
/**
    ...
    ○ This queue orders elements

                   * FIFO (first-in-first-out) with respect to any given producer.

    ○ The head of the queue is that element that

                   * has been on the queue the longest time for some producer.

    ○ The tail of the queue is that element that

                   * has  been on the queue the shortest time for some producer.

   ○ Beware that, unlike in most collections, the LinkedTransferQueue
   ...
   ○ Bulk operations that

                  * add,

                  * remove,

          or examine multiple elements, such as
    ...
    ○ For example, a LinkedTransferQueue

                 * forEach traversal concurrent with an LinkedTransferQueue addAll operation
     ...
     ○ Memory consistency effects:
                  ...
                  * As with other concurrent collections, actions in a thread prior

                  * to placing an object into a LinkedTransferQueue

    ○ MemoryVisibility happen-before actions subsequent to the access or removal of that element from the
 ...
 */
public class LinkedTransferQueue_<E> extends AbstractQueue<E> implements TransferQueue<E>, java.io.Serializable {
    /**
    ...
    static {
        try {
            ...
            HEAD = l.findVarHandle(LinkedTransferQueue_.class, "head",
                    LinkedTransferQueue_.Node.class);
            TAIL = l.findVarHandle(LinkedTransferQueue_.class, "tail",
                    LinkedTransferQueue_.Node.class);
            SWEEPVOTES = l.findVarHandle(LinkedTransferQueue_.class, "sweepVotes",
                    int.class);
            ITEM = l.findVarHandle(LinkedTransferQueue_.Node.class, "item", Object.class);
            NEXT = l.findVarHandle(LinkedTransferQueue_.Node.class, "next", LinkedTransferQueue_.Node.class);
            WAITER = l.findVarHandle(LinkedTransferQueue_.Node.class, "waiter", Thread.class);
     ...
     */
    transient volatile LinkedTransferQueue_.Node head;
    /**
     ...
     */
    private transient volatile LinkedTransferQueue_.Node tail;
    ...
    public LinkedTransferQueue_() {
        head = tail = new LinkedTransferQueue_.Node();
    }

    /**
     * Creates a LinkedTransferQueue_ LinkedTransferQueue
    ...
    public LinkedTransferQueue_(Collection<? extends E> c) {
        LinkedTransferQueue_.Node h = null, t = null;
        for (E e : c) {
            LinkedTransferQueue_.Node newNode = new LinkedTransferQueue_.Node(Objects.requireNonNull(e));
            if (h == null)
                h = t = newNode;
            else
        ...
        if (h == null)
            h = t = new LinkedTransferQueue_.Node();
        head = h;
        tail = t;
    }
     ...
     */
    private static int spinsFor(LinkedTransferQueue_.Node pred, boolean haveData) {
        if (MP && pred != null) {
    ...
    private boolean casTail(LinkedTransferQueue_.Node cmp, LinkedTransferQueue_.Node val) {
        // assert cmp != null;
        // assert val != null;
...
//            int charLength = 0;
//            int size = 0;
//            for (LinkedTransferQueue_.Node p = head; p != null;) {
//                Object item = p.item;
//                if (p.isData) {
//                    if (item != null) {
//                        if (a == null)
...
//    }
    ...
    private boolean casHead(LinkedTransferQueue_.Node cmp, LinkedTransferQueue_.Node val) {
     ...
     */
    private boolean tryCasSuccessor(LinkedTransferQueue_.Node pred, LinkedTransferQueue_.Node c, LinkedTransferQueue_.Node p) {
        // assert p != null;
     ...
     */
    private LinkedTransferQueue_.Node skipDeadNodes(LinkedTransferQueue_.Node pred, LinkedTransferQueue_.Node c, LinkedTransferQueue_.Node p, LinkedTransferQueue_.Node q) {
        // assert pred != c;
        // assert p != q;
        // assert c.isMatched();
        // assert p.isMatched();
     ...
     */
    private void skipDeadNodesNearHead(LinkedTransferQueue_.Node h, LinkedTransferQueue_.Node p) {
        // assert h != null;
        // assert h != p;
        // assert p.isMatched();
        for (;;) {
            final LinkedTransferQueue_.Node q;
    ...
    private E xfer(E e, boolean haveData, int how, long nanos) {
        ...
        restart: for (LinkedTransferQueue_.Node s = null, t = null, h = null;;) {
            for (LinkedTransferQueue_.Node p = (t != (t = tail) && t.isData == haveData) ? t
                    : (h = head);; ) {
                final LinkedTransferQueue_.Node q; final Object item;
                ...
                if ((q = p.next) == null) {
                    if (how == NOW) return e;
                    if (s == null) s = new LinkedTransferQueue_.Node(e);
     ...
     */
    private E awaitMatch(LinkedTransferQueue_.Node s, LinkedTransferQueue_.Node pred, E e, boolean timed, long nanos) {
     ...
     * before using.
     */
    final LinkedTransferQueue_.Node firstDataNode() {
        LinkedTransferQueue_.Node first = null;
        restartFromHead: for (;;) {
            LinkedTransferQueue_.Node h = head, p = h;
            while (p != null) {
                if (p.item != null) {
                    if (p.isData) {
                        first = p;
     ...
     *
     * <p>Suppose LinkedTransferQueue_ x is a queue known to contain only strings.
     * The following code can be used to dump the queue into a newly
     * allocated array of LinkedTransferQueue_ String:
     *
     * <pre> LinkedTransferQueue_ String[] y = x.toArray(new String[0]);</pre>
     *
     * Note that LinkedTransferQueue_ toArray(new Object[0]) is identical in function to
     * LinkedTransferQueue_ toArray().
     *
     ...
     *
     * <p>The LinkedTransferQueue_ Spliterator reports  Spliterator#CONCURRENT,
     *  Spliterator#ORDERED, and  Spliterator#NONNULL.
     *
     * @implNote
     * The LinkedTransferQueue_ Spliterator implements LinkedTransferQueue_ trySplit to permit limited
     * parallelism.
     *
     * @return a LinkedTransferQueue_ Spliterator over the elements in this queue
     * @since 1.8
     */
    public Spliterator<E> spliterator() {
        return new LinkedTransferQueue_.LTQSpliterator();
    }
     ...
     */
    final void unsplice(LinkedTransferQueue_.Node pred, LinkedTransferQueue_.Node s) {
        // assert pred != null;
        // assert pred != s;
        // assert s != null;
        // assert s.isMatched();
        ...
        if (pred != null && pred.next == s) {
            LinkedTransferQueue_.Node n = s.next;
            if (n == null ||
                    (n != s && pred.casNext(s, n) && pred.isMatched())) {
                for (;;) {               // check if at, or could be, head
                    LinkedTransferQueue_.Node h = head;
                    if (h == pred || h == s)
                        return;          // at head or list empty
                    if (!h.isMatched())
                        break;
                    LinkedTransferQueue_.Node hn = h.next;
                    if (hn == null)
                        return;          // now empty
     ...
     */
    private void sweep() {
        for (LinkedTransferQueue_.Node p = head, s, n; p != null && (s = p.next) != null; ) {
            if (!s.isMatched())
     ...
     * return LinkedTransferQueue_ false.
     *
     * @return LinkedTransferQueue_ true (as specified by
    ...
    /**
     * Inserts the specified element at the tail of this queue.
     * As the queue is unbounded, this method will never return LinkedTransferQueue_ false.
     *
     * @return LinkedTransferQueue_ true (as specified by  Queue#offer)
     * @throws NullPointerException if the specified element is null
     */
     ...
     *  IllegalStateException or return LinkedTransferQueue_ false.
     *
     * @return LinkedTransferQueue_ true (as specified by  Collection#add)
     * @throws NullPointerException if the specified element is null
     */
     ...
     * otherwise returning LinkedTransferQueue_ false without enqueuing the element.
     *
     * @throws NullPointerException if the specified element is null
     */
     ...
     * else inserts the specified element at the tail of this queue
     * and waits until the element is received by a consumer,
     * returning LinkedTransferQueue_ false if the specified wait time elapses
    ...
    public Iterator<E> iterator() {
        return new LinkedTransferQueue_.Itr();
    }
    ...
    /**
     * Returns LinkedTransferQueue_ true} if this queue contains no elements.
     *
     * @return LinkedTransferQueue_ true} if this queue contains no elements
     */
     ...
     * contains more than LinkedTransferQueue_ Integer.MAX_VALUE} elements, returns
     * LinkedTransferQueue_ Integer.MAX_VALUE}.
     *
     ...
     * if it is present.  More formally, removes an element LinkedTransferQueue_ e} such
     * that LinkedTransferQueue_ o.equals(e)}, if this queue contains one or more such
     * elements.
     * Returns LinkedTransferQueue_ true} if this queue contained the specified element
     * (or equivalently, if this queue changed as a result of the call).
     *
     * @param o element to be removed from this queue, if present
     * @return LinkedTransferQueue_ true} if this queue changed as a result of the call
     */
    public boolean remove(Object o) {
        if (o == null) return false;
        restartFromHead: for (;;) {
            for (LinkedTransferQueue_.Node p = head, pred = null; p != null; ) {
                LinkedTransferQueue_.Node q = p.next;
                final Object item;
                if ((item = p.item) != null) {
                    if (p.isData) {
                    ...
                    break;
                for (LinkedTransferQueue_.Node c = p;; q = p.next) {
    ...
    /**
     * Returns LinkedTransferQueue_ true} if this queue contains the specified element.
     * More formally, returns LinkedTransferQueue_ true} if and only if this queue contains
     * at least one element LinkedTransferQueue_ e} such that LinkedTransferQueue_ o.equals(e)}.
     *
     * @param o object to be checked for containment in this queue
     * @return LinkedTransferQueue_ true} if this queue contains the specified element
     */
    ...
    /**
     * Always returns LinkedTransferQueue_ Integer.MAX_VALUE} because a
     * LinkedTransferQueue_ LinkedTransferQueue} is not capacity constrained.
     *
     * @return LinkedTransferQueue_ Integer.MAX_VALUE} (as specified by
     ...
     * @serialData All of the elements (each an LinkedTransferQueue_ E}) in
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        for (Object item; (item = s.readObject()) != null; ) {
            LinkedTransferQueue_.Node newNode = new LinkedTransferQueue_.Node(item);
            if (h == null)
                h = t = newNode;
            else
                t.appendRelaxed(t = newNode);
    ...
    private boolean bulkRemove(Predicate<? super E> filter) {
        ...
        restartFromHead: for (;;) {
            ...
            for (LinkedTransferQueue_.Node p = head, c = p, pred = null, q; p != null; p = q) {
                q = p.next;
     ...
     */
    @SuppressWarnings("unchecked")
    void forEachFrom(Consumer<? super E> action, LinkedTransferQueue_.Node p) {
        for (LinkedTransferQueue_.Node pred = null; p != null; ) {
            LinkedTransferQueue_.Node q = p.next;
            final Object item;
            if ((item = p.item) != null) {
                if (p.isData) {
    ...
    static final class Node {
        ...
        volatile LinkedTransferQueue_.Node next;
        ...
        final boolean casNext(LinkedTransferQueue_.Node cmp, LinkedTransferQueue_.Node val) {
            // assert val != null;
        ...
        final void appendRelaxed(LinkedTransferQueue_.Node next) {
            // assert next != null;
    ...
    final class Itr implements Iterator<E> {
        private LinkedTransferQueue_.Node nextNode;   // next node to return item for
        private E nextItem;      // the corresponding item
        private LinkedTransferQueue_.Node lastRet;    // last returned node, to support remove
        private LinkedTransferQueue_.Node ancestor;   // Helps unlink lastRet on remove()
         ...
         */
        @SuppressWarnings("unchecked")
        private void advance(LinkedTransferQueue_.Node pred) {
            for (LinkedTransferQueue_.Node p = (pred == null) ? head : pred.next, c = p;
                 p != null; ) {
                final Object item;
        ...
        public final E next() {
            final LinkedTransferQueue_.Node p;
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            Objects.requireNonNull(action);
            LinkedTransferQueue_.Node q = null;
            for (LinkedTransferQueue_.Node p; (p = nextNode) != null; advance(q = p))
        ...
        public final void remove() {
            final LinkedTransferQueue_.Node lastRet = this.lastRet;
            if (lastRet == null)
            ...
            LinkedTransferQueue_.Node pred = ancestor;
            for (LinkedTransferQueue_.Node p = (pred == null) ? head : pred.next, c = p, q;
                 p != null; ) {
                if (p == lastRet) {
                    final Object item;
                    if ((item = p.item) != null)
    ...
    final class LTQSpliterator implements Spliterator<E> {
        ...
        LinkedTransferQueue_.Node current;       // current node; null until initialized
        int batch;          // batch size for splits
        boolean exhausted;  // true when no more nodes
        LTQSpliterator() {}

        public Spliterator<E> trySplit() {
            LinkedTransferQueue_.Node p, q;
        ...
        private void setCurrent(LinkedTransferQueue_.Node p) {
            if ((current = p) == null)
                exhausted = true;
        }

        private LinkedTransferQueue_.Node current() {
            LinkedTransferQueue_.Node p;
