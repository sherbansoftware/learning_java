package Algorithm_Complexity.Big_O_Notation;

/*                                            Big O notation

    The big O notation is a way of expressing the complexity related to the number of items(steps) that an algorithm has
        to deal with.
    A big O notation gives us a way of comparing the time complexity of different algorithms in an objective manner,
        in a hardware-independent manner.
    It's correct to say O of N  about complexity of Big o notation

    In computer science,

        big O notation is used to classify algorithms according to how their run time or space requirements grow as the
            input size grows.
   ...
   Therefore, to get the most efficient algorithm of all the algorithms described for example as O(N) you need to investigate
    ...
    Constant time complexity   O(1):
            ...
            * If the number of elements is equal to n, the steps to accomplish the task never changes.
