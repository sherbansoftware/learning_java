package Collections.Concurrent_Collection_Classes.CopyOnWriteArrayList;
...
/**                                                 CopyOnWriteArrayList
    ...
    A thread-safe variant of  java.util.ArrayList in which all mutative operations ( add,  set, and so on) are implemented
    ...
    This is ordinarily too costly, but may be more efficient than alternatives when traversal operations vastly outnumber
        ...
        among concurrent threads.The "snapshot" style iterator method uses a reference to the state of the array at the 
        point that the iterator was created.This array never changes during the lifetime of the iterator, so interference 
    ...
    The iterator will not reflect additions, removals, or changes to the list since the iterator was created.
            ...
            * this is not  fail fast: iterator is guaranteed not to throw ConcurrentModificationException
            ...
            * whenever there is an update or write operation, there will be a

                  clone copy that will get created and that update operation or write operation will be performed
                  on that cloned copy rather than on the main object.

            * read operation

                  will be done on the main object and

             * write operation

                   will be done on the cloned object

             * and later these two main objects will get in sync with the clone object internally by JVM.That's the way
                   CopyOnWrite object achieves Trade Safety.

             * For every update, a separate clone copy will get created.So if there are 100 updates, 100 clone copy will
                   get created.So you might have understood by now that this is a performance overhead.
            ...
            * Memory_Address consistency effects: As with other concurrent collections, actions in a thread prior to placing
...
public class CopyOnWriteArrayList_<E>
     ...
     */
    public CopyOnWriteArrayList_(@NotNull Collection<? extends E> c) {
        Object[] elements;
    ...
    /**
     * A version of remove(lang.classes.Object) using the strong hint that given
