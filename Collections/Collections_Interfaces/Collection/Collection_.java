package Collections.Collections_Interfaces.Collection;

import Collections.Collections_Interfaces.List.List_;
import Collections.Collections_Interfaces.Set.Set_;
import Collections.Collections_Iteration.Iterator.Iterator_;
...
/**
    ...
    All general-purpose Collection implementation classes (which typically implement Collection indirectly through one of its
        subinterfaces) should provide two "standard" constructors: a void (no arguments) constructor, which creates an empty
        collection, and a constructor with a single argument of type Collection, which creates a new collection with the
        same elements as its argument.In effect, the latter constructor allows the user to copy any collection, producing
    ...
    The "destructive" methods contained in this interface, that is, the methods that modify the collection on which they
        ...
        invocation would have no effect on the collection.  For example, invoking the {@link #addAll(Collection_)} method
        on an unmodifiable collection may, but is not required to, throw the exception if the collection to be added is empty.

    <a name="optional-restrictions">
        Some collection implementations have restrictions on the elements that they may contain.</a> For example, some
        implementations prohibit null elements, and some have restrictions on the types of their elements.Attempting to
        add an ineligible element throws an unchecked exception, typically NullPointerException or ClassCastException.

    Attempting to query the presence of an ineligible element may throw an exception, or it may simply return false; some
        implementations will exhibit the former behavior and some will exhibit the latter. More generally, attempting an
        operation on an ineligible element whose completion would not result in the insertion of an ineligible element
        into the collection may throw an exception or it may succeed, at the option of the implementation.
        Such exceptions are marked as "optional" in the specification for this interface.

    It is up to each collection to determine its own synchronization policy.In the absence of a stronger guarantee by the
        implementation, undefined behavior may result from the invocation of any method on a collection that is being
    ...
    Many methods in Collections_ Framework interfaces are defined in terms of the {@link Object#equals(Object) equals} method.
        For example, the specification for the {@link #contains(Object) contains(lang.classes.Object o)} method says: "returns
        true if and only if this collection contains at least one element e such that (o==null ? e==null : o.equals(e))."
    This specification should not be construed to imply that invoking Collection.contains with a non-null argument o will
        cause o.equals(e) to be invoked for any element e.Implementations are free to implement optimizations whereby the
        equals invocation is avoided, for example, by first comparing the hash codes of the two elements.(The {@link
        Object#hashCode()} specification guarantees that two objects with unequal hash codes cannot be equal.)  More generally,
        implementations of the various Collections_Class Framework interfaces are free to take advantage of the specified behavior
        of underlying {@link Object} methods wherever the implementor deems it appropriate.
    ...
    Some collection operations which perform recursive traversal of the collection may fail with an exception for self-referential
        instances where the collection directly or indirectly contains itself. This includes the {@code clone()}, {@code equals()},
        {@code hashCode()} and {@code toString()} methods.Implementations may optionally handle the self-referential scenario,
         however most current implementations do not do so.

    This interface is a member of the href="{@docRoot}/../technotes/guides/collections/index.html" Java Collections_Class Framework.

    @implSpec
    The default method implementations (inherited or otherwise) do not apply any synchronization protocol. If a {@code Collection}
        implementation has a specific synchronization protocol, then it must override default implementations to apply that
        protocol.
...
public interface Collection_<E> extends Iterable<E> {
    ...
    */
     ...
     *
     * @return an Iterator_Interface over the elements in this collection
     */
     Iterator<E> iterator();
     ...
     *
     * Collections_ that support this operation may place limitations on what
     ...
     * general contract for the lang.classes.Object.equals, programmers who
     ...
     * lang.classes.Object.equals.  It is not necessary to do so, and the simplest
     * course of action is to rely on lang.classes.Object's implementation, but
     * the implementor may wish to implement a "value comparison" in place of
     * the default "reference comparison."  (The List and
     * Set interfaces mandate such value comparisons.)
     *
     * The general contract for the lang.classes.Object.equals method states that
     ...
     * contract for the lang.classes.Object.hashCode method, programmers should
     * take note that any class that overrides the lang.classes.Object.equals
     * method must also override the lang.classes.Object.hashCode method in order
     * to satisfy the general contract for the lang.classes.Object.hashCode method.
