package Collection.Collection_Interfaces.List;

import Collection.Collection_Interfaces.Collection.Collection_;
import Collection.Collection_Iteration.ListIterator.ListIterator_;
import Collection.Collection_Interfaces.Comparator.Comparator_;
...
/**                                              List interface
    ...
    The user of this  interface has precise control over where in the list each element is inserted.The user can access 
    ...
    More formally, lists typically allow pairs of elements e1 and e2  such that e1.equals(e2), and they typically allow 
        multiple  null elements if they allow null elements at all.It is not inconceivable that someone might wish to 
        implement a list that prohibits duplicates, by  throwing runtime exceptions when the user attempts to insert them, 
    ...
    The List interface places additional stipulations, beyond those  specified in the Collection interface, on the contracts
        of the  iterator, add, remove, equals, and  hashCode methods.
    ...
    CopyOnWriteArrayList
     ...
     CopyOnWriteArraySet.
         ...
         No synchronization is necessary, even during iteration, and iterators are guaranteed never to throw
             ConcurrentModificationException. This implementation is well suited to maintaining event-handler lists,
             in which change is infrequent, and traversal is frequent and potentially time-consuming.

                                              If you need synchronization,

    a Vector will be slightly faster than an ArrayList synchronized with Collections_Class.synchronizedList.
     ...
     See
         ...
         in the Convenience Implementations section for more information.
            ...
            * Primitive types are automatically converted to corresponding wrapper types, e.g.
                  integer numbers are converted to Integer objects.
            ...
            * that these operations may execute in time proportional to the index value for some implementations (the 
                  LinkedList class, for example). Thus, iterating over the elements in a list is typically  preferable 
            ...
            * The List interface provides a special iterator, called a  ListIterator, that allows element insertion  and 
                  replacement, and  bidirectional access in addition to the normal operations that the Iterator Interface 
                  interface provides.A method is provided to obtain a list iterator that starts at a specified position
                  in the list

             * The List interface provides two methods to search for a specified object. From a performance standpoint,
                   these methods should be used with caution.In many implementations they will perform costly linear searches.

             * The List interface provides two methods to efficiently insert and remove multiple elements at an arbitrary
                   point in the list.

             * While it is permissible for lists to contain themselves as elements,extreme caution is advised: the equals
                   and hashCode methods are no longer well defined on such a list.

             * Some list implementations have restrictions on the elements that they may contain. For example, some
                   ...
                   or it may simply return false; some implementations will exhibit the former  behavior and some will
                   exhibit the latter.More generally, attempting an operation on an ineligible element whose completion
                   would not result in the insertion of an ineligible element into the list may throw an exception or it
                   may succeed, at the option of the implementation.

              * Such exceptions are marked as "optional" in the specification for this  interface.

              * So if you are going to implement a custom list, it's a good idea to extend one of these two classes.

                        AbstractList,
                        AbstractSequentialList

                    rather than start with list itself.
...
public interface List_<E> extends Collection<E> {
     ...
     * @return true (as specified by {@link Collection_#add})
     ...
     * <pre>{@code
     *     final ListIteratorDemo<E> li = list.listIterator();
     *     while (li.hasNext()) {
    ...
    default void sort(Comparator<? super E> c) {
       Object[] a = this.toArray();
        ...
        ListIterator<E> i = (ListIterator<E>) this.listIterator();
        for (Object e : a) {
            i.next();
            i.set((E) e);
        }
     ...
     * Collections_ class can be applied to a subList.<p>
     *
     ...
     * from the list's {@code Iterator_Interface}.  The spliterator inherits the
