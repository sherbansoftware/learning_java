...
/*                                            Functional Interface:
    ...
    A comparison function, which imposes a total ordering on some collection of objects.Comparators can be passed to a sort
        method (such as Collections.sort or Arrays.sort) to allow precise control over the sort order.Comparators can also
        be used to control the order of certain data structures (such as sorted sets or sorted maps), or to provide an
    ...
    The ordering imposed by a comparator c on a set of elements S is said to be
            ...
            consistent with equals if and only
                    ...
                    if c.compare(e1, e2)==0 has the same boolean value as
                    e1.equals(e2)
