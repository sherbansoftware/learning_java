...
//  https://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.12
 ...
 /*                                                A variable is
    ...
    ? A variable's value is changed by

                  * an assignment

                  * or by a prefix or postfix ++ (increment) or -- (decrement) operator

    ? Compatibility of the value of a variable with its type is GUARANTEED by the design of the Java programming
          language, as long as a program does not give rise to compile-time unchecked warnings. Default values
    ...
    ? A variable of an interface type:
                  ...
                  * or a reference to any instance of any class that implements the interface.
    ...
    ? can only occur if the program:

                  * performed some operation involving a raw type that would give rise to a compile-time unchecked warning

                  * or if the program aliases an array variable of non-reifiable element type through an array variable
                        of a supertype which is either raw or non-generic.
    ...
    There are eight kinds of variables:
        ...
        2. An instance variable:
                ...
                If a class T has a field a that is an instance variable, then a new instance variable a is created and
                    initialized to a default value (§4.12.5) as part of each newly created object of class T or of any
                    class that is a subclass of T (§8.1.4). The instance variable effectively ceases to exist when the
                    object of which it is a field is no longer referenced, after any necessary finalization of the object
                    (§12.6) has been completed.

        3. Array components:

               are unnamed variables that are created and initialized to default values (§4.12.5) whenever a new object
               that is an array is created (§10 (Arrays), §15.10.2). The array components effectively cease to exist
