...
/*                                    Functional Interfaces
    ...
    ○ A functional interface is an interface:
                   ...
                   * that has just one ABSTRACT method (aside from the methods of Object),
      ...
      This "single" method may take the form of multiple abstract methods with override-equivalent signatures inherited
           from superinterfaces;
                  ...
                  * in this case, the inherited methods logically represent a single method.

    ○ For an interface I, let M be the set of abstract methods that are members of I that do not have the same signature
          as any public instance method of the class Object. Then,

    ○ I is a Functional Interface if there exists a method m in M for  which both of the following are true:
    ...
    ○ In addition to the usual process of creating an interface instance by declaring and instantiating a class (§15.9),
       ...
       INSTANCES of Functional Interfaces can be created with:
                 ...
                 * and Lambda Expressions (§15.13, §15.27).
    ...
    ○ The definition of functional interface excludes methods in an interface that are also public methods in Object.
       This is to allow functional treatment of an interface like
                ...
                * java.util.Comparator<T> that declares multiple abstract  methods of which only one is really "new"

                     - int compare(T,T).

             The other method - boolean equals(Object) - is an explicit declaration of an abstract method that would
             otherwise be implicitly declared, and will be automaticallyimplemented by every class that implements
             the interface.
    ...
    ○ Note that if non-public methods of Object, such as clone(), are declared in an interface, they are not automatically
     ...
     The implementation inherited from Object
               ...
               * while the interface method is necessarily public.

           The only way to implement such an interface would be for a class to override the non-public Object method
...
*/
public class Functional_Interfaces_ {
    public static void main(String[] args) {
        ...
        new TreeSet<String>((s1, s2) -> {
            return s1.length() - s2.length();
        });

        //  (parameters) -> expressions
        new TreeSet<String>((s1, s2) -> s1.length() - s2.length());
