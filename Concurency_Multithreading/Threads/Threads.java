...
/*                                                   THREAD
      ...
      Our code RUN in MAIN THREAD or in other THREADS that we explicitly create.
                  ...
                  * and has his thread STACK MEMORY which only that thread can access.
    ...
    Threads with higher priority are executed in preference to threads with lower priority. Each thread may or may not
        also be marked as a daemon. When code running in some thread creates a new Thread object, the new thread has i
        ts priority initially set equal to the priority of the  creating thread, and is a daemon thread if and only if
    ...
    By multitasking quickly between different threads our computer can create an illusion that all those tasks are actually
        happening in the same time.

    The therm we use for this kind of multitasking is concurrency.
        Not as that, we don't even need multiple cores to achieve concurrency.Even with one core, we can create responsive
            applications by using multiple threads.
