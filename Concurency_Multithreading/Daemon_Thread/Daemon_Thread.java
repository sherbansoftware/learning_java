package Concurency_Multithreading.Daemon_Thread;

/*                                                      Daemon threads

    are threads that run in the background and do not prevent our application from exiting if the main thread terminates.


    If the main thread terminates a few scenarios, when we want to create our threads as Daamon
            ...
            *  Background tasks that should not block our application from terminating
             ...
             * A second scenario is when we do not have control over the code, being executed in the worker thread and we
                  ...
                  Example: Worker thread thst uses an external library
