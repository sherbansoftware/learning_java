package Variables.Variable.Final_Variable;

/*                                                        final Variables

    A variable can be declared final. A final variable may only be assigned to once. It is a compile-time error if a final
    ...
    Once a final variable has been assigned, it always contains the same value. If a final variable holds a reference to
        ...
        reference to an array, then the components of the array may be changed by operations on the array, but the
        variable will always refer to the same array.

    A blank final is a final variable whose declaration lacks an initializer.

    A constant variable is a final variable of primitive type or type String that is initialized with a constant expression
       (§15.28). Whether a variable is a constant variable or not may have implications with respect to class initialization
    ...
    Three kinds of variable are implicitly declared final:
            ...
            * a field of an interface (§9.3),
            * a local variable which is a resource of a try-with-resources statement (§14.20.3),
            * and an exception parameter of a multi-catch clause (§14.20).
...
class Point {
    ...
    the class Point declares a final class variable origin. The origin variable holds a reference to an object that is an
        instance of class Point whose coordinates are (0, 0). The value of the variable Point.origin can never change, so
        it always refers to the same Point object, the one created by its initializer. However, an operation on this Point
        object might change its state - for example, modifying its useCount or even, misleadingly, its x or y coordinate.

                  Certain variables that are not declared final are instead considered effectively final:

            * A local variable whose declarator has an initializer(§14.4.2) is effectively final if all the following are true:

                    •It is not declared final.

                    •It never occurs as the left hand side in an assignment expression (§15.26). (Note that the local
                         variable declarator containing the initializer is not an assignment expression.)

                    • It never occurs as the operand of a prefix or postfix increment or decrement operator (§15.14, §15.15).

              * A local variable whose declarator lacks an initializer is effectively final if all the following are true:

                     • It is not declared final.

                      • Whenever it occurs as the left-hand side in an assignment expression, it is definitely unassigned
                          and not definitely assigned before the assignment; that is, it is definitely unassigned and not
                          definitely assigned after the right hand side of the assignment expression (§16 (Definite Assignment)).
