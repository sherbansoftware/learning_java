package About_Java.JVM_JDK_JRE.JVM;

// https://www.baeldung.com/jvm-vs-jre-vs-jdk#jvm
...
/*                                                   JVM
    ...
    The JVM first interprets the bytecode. It then stores the class information in the memory area. Finally, it executes
    ...
    Initial tasks of the JVM includes
    ...
    * Method Area  - also known as permanent generation space (PermGen)
    ...
    * Stack area
            ...
            Each frame contains three parts:
                    ...
                    * Local Variable Array_ – contains all the local variables and parameters of the method

                    * Operand Stack – used as a workspace for storing intermediate calculation's result

                    * Frame Data – used to store partial results, return values for methods, and reference to the Exception
    ...
    * Native method stacks
    ...
    Execution engine executes the instructions using information present in the memory areas. It has three parts:
        ...
        * Interpreter
              ...
              Once classloaders load and verify bytecode, the interpreter executes the bytecode line by line. This execution
                  is quite slow. The disadvantage of the interpreter is that when one method is called multiple times, every
                  time new interpretation is required.
        ...
        * Just-In-Time (JIT) Compiler
              ...
              JIT compiler compiles the bytecode of the often-called methods into native code at run-time. Hence it is
                  responsible for the optimization of the Java programs.
              JVM automatically monitors which methods are being executed.Once a method becomes eligible for JIT compilation,
                  it is scheduled for compilation into machine code. This method is then known as a hot method. This
                  compilation into machine code happens on a separate JVM thread.

              As a result, it does not interrupt the execution of the current program. After compilation into machine code,
                  it runs faster.

        * Garbage Collector

             Java takes care of memory management using Garbage Collection. It's a process of looking at heap memory,
                 identifying which objects are in use and which are not, and finally deleting unused objects.
             GC is a daemon thread. It can be called explicitly using System.gc() method, however, it won't be executed
                 immediately and the JVM decides when to invoke GC.

                                                    4. Java Native Interface

    It acts as an interface between the Java code and the native (C/C++) libraries.

    There are situations in which Java alone doesn't meet the needs for your application, for example,

        implementing a platform-dependent feature.

    In those cases, we can use JNI to enable the code running in the JVM to call. Conversely, it enables native methods
        to call the code running in the JVM.
